...
import type { Reducer } from '@illia-web-dev/react-utils/dist/EECtx/store';
 ...
 *   in place typing them with exact Prps type that component would \
 *   expect
...
export type State = {
  portals: Array< WithProps & { Comp: PortalComp; id: string } >;
};
export const defaultState: State = { portals: [] };


export const aTypes = {
  pushPortal: '__illia-web-dev__/eecontext-portal/push-portal',
  popPortal: '__illia-web-dev__/eecontext-portal/pop-portal',
  popAllPortals: '__illia-web-dev__/eecontext-portal/pop-all-portals',
  pushProps: '__illia-web-dev__/eecontext-portal/push-props',
  popProps: '__illia-web-dev__/eecontext-portal/pop-props',
  replaceProps: '__illia-web-dev__/eecontext-portal/replace-props',
} as const;
...
export type Actions = {
  pushPortal: {
    ...
    payload: SafeOmit< State[ 'portals' ][ 0 ], 'props' | 'id' > & {
      props?: Props[];
      id?: string;
    };
  };
  ...
  switch ( action.type ) {
    ...
    case aTypes.pushPortal: {
      const fullPortal: State[ 'portals' ][ 0 ] = {
        ...
        id: action.payload.id || String( Math.random() ),
      };
